def give_bmi(height: list[int | float],
    ...
    """
    Calculates the BMI (Body Mass Index) for each person in the list.
    """
    if len(height) != len(weight):
        raise ValueError(
            "The length of the height and weight lists must be the same"
        )

    if not all(isinstance(h, (int, float)) for h in height):
        raise ValueError(
            "The height list must contain only integers or floats"
        )

    if not all(isinstance(w, (int, float)) for w in weight):
        raise ValueError(
            "The weight list must contain only integers or floats"
        )
